...
{
$vmName=$vm.name
$backupTag=(Get-TagAssignment -Entity $vm -Category $vmBkpCtgyName).Tag.Name
$prnxTag=(Get-TagAssignment -Entity $vm -Category $vmPrnxCtgyName).Tag.Name
    ...
    Log-Write -LogPath $LogFile -LineValue "check if pernixdata tag from vm $vmName is WB"
        ...
        {
            Log-Write -LogPath $LogFile -LineValue "$vmName has vmware tag WB, execute Set-PrnxAccelerationPolicy -WriteThrough -WaitTimeSeconds 60 -Name $vmName"
            Try
            {
            Set-PrnxAccelerationPolicy -WriteThrough -WaitTimeSeconds 60 -Name $vmName
            }
            Catch
            {
            Log-Error -LogPath $LogFile -ErrorDesc $Error[0] -ExitGracefully $False
